...
const Settings = ( { attributes, setAttributes, itemsHandler } ) => {
    ...
    const [ genresList, setGenresList ] = useState( JSON.parse( localStorage.getItem( 'tmdbGenres' ) ?? '{}' ) )
    const loadGenresList = async () => {
        ...
        await fetch( `https://api.themoviedb.org/3/genre/tv/list?api_key=${APIKEY}` )
            .then( response => response.json()
    ...
    const loadCombinedCredits = () => {
        if ( searchPerson.id ) {
            fetch( `https://api.themoviedb.org/3/person/${searchPerson.id}/combined_credits?api_key=${APIKEY}` )
                .then( response => response.json() )
    ...
    const filterCredits = () => _.sortBy(
        _.filter(
            credits[ creditsType ] ?? [],
            credit => {
                const isMediaType = mediaType === credit.media_type
                const isJob = "crew" === creditsType ? credit.job === creditsJob : true
    ...
    const addSelection = () => itemsHandler.add( selection.map( credit_id => {
        const credit = _.findWhere( credits[ creditsType ] ?? [], { credit_id: credit_id } )
        if ( credit ) {
            return {
                title: "tv" === credit.media_type ? credit.name : credit.title,
		        tmdb_id: credit.id,
            }
        }
    } ) )
    ...
    useEffect( () => {
        if ( showSelection ) {
            setFilteredCredits( selection.map( credit_id => _.findWhere( credits[ creditsType ] ?? [], { credit_id: credit_id } ) ) )
        } else {
            setFilteredCredits( filterCredits() )
        }
        return () => {}
    ...
    return (
        <>
            ...
            { showModal && (
                <Modal
                    title={ sprintf( __( "Select Movies/TV Shows from %s", "cinemarathons" ), searchPerson.name ) }
                        ...
                        <FlexItem>
                            <ButtonGroup>
                                <Button
                                    ...
                                    iconSize={ 16 }
                                    onClick={ () => setCreditsType( "cast" === creditsType ? "crew" : "cast" ) }
                                >
                                ...
                                <Button
                                    ...
                                    iconSize={ 16 }
                                    onClick={ () => setCreditsType( "crew" === creditsType ? "cast" : "crew" ) }
                                >
                        ...
                        <FlexItem>
                            <ButtonGroup>
                                <Button
                                    ...
                                    icon={ "movie" === mediaType ? closeSmall : check }
                                    iconSize={ 16 }
                                    onClick={ () => setMediaType( "movie" === mediaType ? "tv" : "movie" ) }
                                />
                                <Button
                                    className="has-text"
                                    variant="secondary"
                                    ...
                                    icon={ "tv" === mediaType ? closeSmall : check }
                                    iconSize={ 16 }
                                    onClick={ () => setMediaType( "tv" === mediaType ? "movie" : "tv" ) }
                                />
                            </ButtonGroup>
                        </FlexItem>
                        { "crew" === creditsType && (
                    ...
                    <div className="editor-table">
                        <div className="editor-header">
                            <div className="header-column check-column">
                                <CheckboxControl
                                    checked={ filteredCredits.length && filteredCredits.length === selection.length }
                                    onChange={ value => setSelection( value ? _.pluck( filteredCredits, "credit_id" ) : _.difference( selection, _.pluck( filteredCredits, "credit_id" ) ) ) }
                                    indeterminate={ filteredCredits.length && filteredCredits.length !== selection.length }
                        ...
                        <div className="editor-content">
                            { filteredCredits.length ? filteredCredits.map( ( credit, key ) => (
                                <div
                                    key={ key }
                                    className="list-item"
                                >
                                    <div className="list-item-column column-watched check-column">
                                        <CheckboxControl
                                            checked={ -1 < _.indexOf( selection, credit.credit_id ) }
                                            onChange={ value => setSelection( value ? _.unique( [ ...selection, credit.credit_id ] ) : _.without( selection, credit.credit_id ) ) }
                                        />
                                    </div>
                                    <div className="list-item-column column-title text-column">
                                        { "tv" === credit.media_type && (
                                            <>
                                                ...
                                                <div>
                                                    { "crew" === creditsType && (
                                                        <span>{ __( credit.job, "cinemarathons" ) }</span>
                                                    ) }
                                                    { credit.first_air_date ? (
                                                        <span>{ ( new Date( credit.first_air_date ) ).getFullYear() }</span>
                                                    ) : (
                                                        <span>?</span>
                                                    ) }
                                                    { credit.genre_ids && (
                                                        <span>{ _.pluck( credit.genre_ids.map( id => _.findWhere( genresList[ credit.media_type ] ?? [],{ id: id } ) ), 'name' ).join( ', ' ) }</span>
                                                    ) }
                                                </div>
                                            </>
                                        ) }
                                ...
                                <div className="list-item"><p>{ __( "Nothing matches your selection. Maybe try some other filters?", "cinemarathons" ) }</p></div>
                            ) }
                        </div>
                        <div className="editor-footer">
                            <div className="footer-content">
                                { sprintf( _n( '%s Item, %d Selected', '%s Items, %d Selected', credits.cast?.length ), credits.cast?.length, selection.length ) }
                            </div>
                            <div className="footer-actions">
                                <ToggleControl
                                    label={ __( "View Selection", "cinemarathons" ) }
                                    checked={ showSelection }
                                    onChange={ () => setShowSelection( ! showSelection ) }
