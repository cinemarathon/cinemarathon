...
const EntrySelector = ( { entry, setEntry, closeModal } ) => {
    ...
    const { searchResults } = useSelect(
		( select ) => {
			let searchResults = []
			if ( search ) {
				searchResults = select( coreStore ).getEntityRecords( "postType", "post", {
					per_page: 10,
    ...
    const select = () => {
        dispatch( "core/notices" ).createNotice(
            "success",
            __( "Journal Entry selected properly. Don't forget to save your post!", "" ),
            {
                type: "snackbar",
                isDismissible: true,
                actions: [
    ...
    const post = useSelect( ( select ) => {
        if ( postID ) {
            const post = select( coreStore ).getEntityRecord( 'postType', 'post', postID, {
                _embed: 1,
                context: "view"
            } ) ?? {}
            ...
            return {
                ...
                categories: filter( flatten( ( post._embedded || {} )["wp:term"] || [] ) ?? [], { taxonomy: "category" } ) ?? []
            }
        }
        return {}
    }, [ postID ] )
        ...
        <>
            <ComboboxControl
                ...
                __experimentalRenderItem={ ( { item } ) => {
                    ...
                    return (
                        <div>
                            <div style={ { marginBottom: '0.2rem' } }><strong>{ label }</strong></div>
                            <small>{ sprintf( __( "Published %s", "cinemarathons" ), date ) }</small>
                        </div>
                    )
                } }
            />
